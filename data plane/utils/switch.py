...
class SwitchConnection(object):
    ...
    def MasterArbitrationUpdate(self, electionSeriesNumber,dry_run=False, **kwargs):
        request = p4runtime_pb2.StreamMessageRequest()
        request.arbitration.device_id = self.device_id
        request.arbitration.election_id.high = int(electionSeriesNumber /(2**16))
        request.arbitration.election_id.low = electionSeriesNumber %(2**16)
    ...
    def WriteTableEntry(self, electionSeriesNumber,table_entry, type, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.high = int(electionSeriesNumber /(2**16))
        request.election_id.low = electionSeriesNumber %(2**16)
    ...
    def DeleteTableEntry(self, electionSeriesNumber,table_entry, dry_run=False):
        request = p4runtime_pb2.WriteRequest()
        request.device_id = self.device_id
        request.election_id.high = int(electionSeriesNumber /(2**16))
        request.election_id.low = electionSeriesNumber %(2**16)
    ...
    def ReadTableEntry(self, table_entry):
        request = p4runtime_pb2.ReadRequest()
        request.device_id = self.device_id
        entity = request.entities.add()
        entity.table_entry.table_id = table_entry.table_id
        entity.table_entry.match.extend(table_entry.match)
        for response in self.client_stub.Read(request):
            yield response
